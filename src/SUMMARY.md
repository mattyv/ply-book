# Summary

[Introduction](introduction.md)
[Set up your workshop](setup.md)
[The scheduler we will build](project.md)

- [Turn a requirement into a claim](01-first-claim.md)
- [Follow a failure back to the code](02-first-failure.md)
- [Challenge a passing result](03-evidence-limits.md)
- [Find an input the tests missed](05-missed-input.md)
  - [Read the evidence](read-the-evidence.md)
- [Independent challenge: cancellation and retries](04-retry-eligibility.md)

[Worked solutions](solutions.md)
[Where to go next](next.md)
